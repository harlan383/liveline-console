...
MIN_BBR_ENABLE_DRY_RUN_VERSION = "0.1.41-stage-3.3.206-bbr-sysctl-sandbox-fix"
MIN_BBR_ENABLE_DRY_RUN_VERSION_KEY = (0, 1, 41, 3, 3, 206)
MIN_BBR_ENABLE_REAL_EXECUTION_VERSION = "0.1.41-stage-3.3.206-bbr-sysctl-sandbox-fix"
MIN_BBR_ENABLE_REAL_EXECUTION_VERSION_KEY = (0, 1, 41, 3, 3, 206)
...
MIN_TRANSIT_ROUTE_HAPROXY_TCP_VERSION = "0.1.36-stage-3.3.188-transit-port-approval"
MIN_TRANSIT_ROUTE_HAPROXY_TCP_VERSION_KEY = (0, 1, 36, 3, 3, 188)
...
def minimum_worker_version_for_command(command_type: str | None) -> str:
    ...
    if command_type == "bbr_enable_real_execution":
        return MIN_BBR_ENABLE_REAL_EXECUTION_VERSION
...
def minimum_worker_version_key_for_command(command_type: str | None) -> tuple[int, int, int, int, int, int]:
    ...
    if command_type == "bbr_enable_real_execution":
        return MIN_BBR_ENABLE_REAL_EXECUTION_VERSION_KEY
    if command_type == "transit_readonly_preflight":
