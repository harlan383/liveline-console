...
| Stage 3.3.181 Xray v25.5.16 default and multi-inbound preserve | Record the production Reality invalid-connection fix, pin LiveLine-managed Xray-core to v25.5.16, upgrade older managed binaries safely, and preserve existing inbounds when adding dynamic landing ports; Worker sha256 `f045c96bae690dbfbf07fa03cfa6288b882de1ee38340a69e8696467c75cf379`; no real node, listener, remote upgrade, cutover, firewall, or share-link mutation |
| Stage 3.3.182 Xray temp config JSON suffix hotfix | Ensure LiveLine-managed Xray temporary config test files end with `.json` for Xray v25.5.16 format detection; Worker 0.1.35 sha256 `76ce855d0b63d03fdf53261d23030beb9e0f990d04d72689b098927f09986e83`; no real node, listener, remote upgrade, cutover, firewall, or share-link mutation |
| Stage 3.3.183 Production success record | Record the production success of Xray-core v25.5.16, Worker 0.1.35, the Xray temp config JSON suffix hotfix, and the new 28917/TCP direct Reality node; no code, binary, remote execution, listener, firewall, or share-link mutation |
| Stage 3.3.184 Old leaked node and stale worker cleanup plan | Plan safe cleanup for the leaked 27940 test node, stale Worker records, and historical failed tasks while protecting the production 28917/TCP Reality node; no code, binary, remote execution, deletion, firewall, docker-compose, or share-link mutation |
| Stage 3.3.185 Old leaked node safe-delete approval no-op | Record read-only approval that leaked 27940 is already deleted and absent from Xray config, so no remote cleanup is required while protecting the production 28917/TCP node; no code, binary, database mutation, remote execution, deletion, firewall, docker-compose, or share-link mutation |
| Stage 3.3.186 Stale Worker record visibility plan | Plan UI and audit handling for stale historical Worker records while protecting the current 0.1.35 landing Worker and production 28917/TCP Reality node; no code, database mutation, remote execution, deletion, firewall, docker-compose, or share-link mutation |
| Stage 3.3.187 Remove node connectivity placeholder | Remove the misleading direct-node “connectivity: pending” UI placeholder because LiveLine does not automatically verify local client connectivity; keep service status, client-link copy, QR, and node actions unchanged; no backend migration, Worker rebuild, remote execution, firewall, or share-link mutation |
| Stage 3.3.188 Transit route planned listen port approval payload fix | Pass approved planned listen port and firewall confirmation into HAProxy TCP transit route Worker payloads, bump Worker to 0.1.36 for dynamic HAProxy port approval, and rebuild the bundled binary; no real route creation, listener, SSH, firewall, cutover, docker-compose, or share-link mutation |
| Stage 3.3.189 Transit route real-create follow-up flow fix | Require explicit UI confirmation after HAProxy TCP dry-run/final approval, then create the protected real_create Worker command with matching dry-run payload and Worker binding; no Worker change, real route creation, listener, SSH, firewall, cutover, docker-compose, or share-link mutation |
| Stage 3.3.190 Remove fixed HAProxy real-create parameter gate | Replace the residual fixed 23843/27939 HAProxy TCP real_create gate with dynamic dry-run approval field validation; no Worker change, real route creation, listener, SSH, firewall, cutover, docker-compose, or share-link mutation |
| Stage 3.3.191 Transit route result processor dynamic approval fix | Replace the residual fixed-port transit_route_create result persistence gate with dynamic command/result approval matching for HAProxy TCP real_create; no Worker change, binary rebuild, real route creation, listener, SSH, firewall, cutover, docker-compose, or share-link mutation |
| Stage 3.3.192 Transit route display name preserve | Preserve the user-entered transit route display name separately from the internal HAProxy route_name and service_name so `transit_routes.name` and candidate export remarks use the display name; no Worker change, binary rebuild, real route creation, listener, SSH, firewall, cutover, docker-compose, or share-link mutation |
| Stage 3.3.193 Transit route name edit button | Add a safe transit-route edit action that only renames `transit_routes.name` for display/export remarks while leaving listen port, target, forwarding method, service, HAProxy config, share_link, cutover, Worker, and firewall state unchanged |
...
| Stage 3.4.28 Advanced debug protected resource registration approval | Add an advanced-debug approval dry-run after the protected resource registration dry-run; no resource, WorkerCommand, TransitRoute, HAProxy route, SSH, firewall, cutover, ordinary product UI, or share-link mutation |
| Stage 3.4.29 Protected resource registration command create | Create a local protected pending registration task record after approval dry-run; no real resources, Worker remote execution, TransitRoute, HAProxy route, listener, SSH, firewall, cutover, ordinary UI, Worker, migration, docker-compose, or share-link mutation |
| Stage 3.4.30 Protected resource registration execution verify | Execute and verify approved local DB registration from a Stage 3.4.29 pending command, creating or reusing only local transit resource and landing node records; no WorkerCommand, TransitRoute, HAProxy route, listener, SSH, firewall, cutover, ordinary UI, Worker, migration, docker-compose, or share-link mutation |
